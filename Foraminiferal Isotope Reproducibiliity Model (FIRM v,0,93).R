...
## Range of possible depths ##
  depthrange<-c(0,70) #Upper and lower bound for depths in meters

##Location Choice##
  lon<-161 #longitude (not longitude currently, but cell within Levitus dataset)
  lat<-88 #latitude (not value currently, but cell within Levitus dataset)
    ##default Site 803 s.lon[161],s.lat[88]
...
##  Diagenesis?  ##
  diag.ind<-round(1/10*ind) #Number of individuals diagenetically altered 
...
## 'Vital Effect' value ##
  err<-0.089
    #Here using the standard error from the Bemis et al 1998
    #That error is ~0.3-0.4degC ~= .089permil using erez&luz

##  Misidentification  ##
  misid<-0.1 #0-100% chance of including a misidentified bug
...
#Seasonality
  time<-c(2,3,4) #Seasons to choose from (1,2,3,4)
    #Example syntax: time<-c(2,3,4) OR time<-c(1)
  weights<-c(20,50,30) #chance that an individual grew in each season (integer, 1 to 100 , should sum to 100)
...
#Plot for Temperature and Salinity ####
  ...
  par(mfcol=c(1,3))
  ### NOTE only plots first season profile
  plot(temp1,
     t.depth[1:4],
     type='p',
     ylim=c(5500,0),
  ...
  plot(salt1, 
     ...
     pch=16,
     lwd=2)
  lines(salt1,s.depth[1:4],col='red',lwd=2)
  ...
  for(A in 1:reps){
    ...
    #Calculating pure oxygen isotope signal for each individual ####
      ...
      if(equation == "kim&oneil"){
        d18O<-{
          {d18O.wd+-0.27}-{20/91}*temp.s+ {46/13} #-0.27 is to convert VSMOW to VPDB
        }  
      }
      if(equation == "mielke"){
        d18O<-{
